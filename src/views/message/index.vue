<template>
  <div class="app-container">
    <!-- 留言列表 -->
    <el-table v-loading="listLoading" :data="data" element-loading-text="Loading" fit highlight-current-row>
      ...
      <el-table-column label="创建日期" align="center" width="230" prop="createDate" />
...
<script>
import { getMessages, removeMessage } from '@/api/message.js'
...
export default {
  ...
  methods: {
    ...
    // 删除留言
    handleDelete({ id }) {
      this.$confirm('是否删除此条留言信息？', '提示', { type: 'warning' }).then(() => {
        this.$message({ type: 'success', message: '删除成功！' })
        removeMessage(id).then((res) => this.fetchData())
      }).catch(() => {
